...
class OutputParser2(DeviceParams):  
    ...
    def ac(self, file_name):
        try:
            # Use working directory or default directory
    ...
    def analyze_amplifier_performance(self, vinp, vout, time, d0):
        ...
        def get_step_parameters(vinp, time):
            """Extract step parameters from input signal"""
            dv = np.diff(vinp)
        ...
        if v0 is None:  # If parameter extraction failed
        ...
        def find_settling_time_index(delta, d0):
            """Find index where signal settles within tolerance"""
        ...
        def get_slope_and_settling_time(vout, time, v0, v1, start_t, end_t, d0, mode):
            """Calculate slew rate and settling time"""
    ...
    def extract_tran_data(self,file_name):
        """Extract transient simulation data from file"""
        time_points = []
        raw_data = []
        vin_data = []
        vout_data = []
...
def aggregate_rewards(results_dict, attention_weights):
    ...
    Args:
        results_dict: {corner_idx: {'reward': float}}
